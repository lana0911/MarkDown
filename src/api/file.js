...
export default {
  ...
  //File (.md)
  GetFile(file_id){ //get 單一資料(md)
    return http({
      url: `/mdapi/api/markdown/file/${file_id}`,
      method: 'get',
    })
  },
  UpdateFile(data, file_id, encoding){ //get 單一資料(md)
  ...
  AddFile(data, catlog_id, encoding){ //get 單一資料(md)
    var encode = encoding ? 1 : 0
    return http({
  ...
  DeleFile(id){ //get 單一資料(md)
    return http({
      url: `/mdapi/api/markdown/file/md&img/${id}`,
      method: 'delete',
    })
  },
  DownloadFile(id){ //get 單一資料(md)
    return http({
      url: `/mdapi/api/markdown/file/md&img/${id}`,
      method: 'get',
      // header: {
